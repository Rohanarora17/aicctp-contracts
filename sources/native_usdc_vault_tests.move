...
module yieldflow::native_usdc_vault_fa_tests {
    ...
    public fun test_initialize_and_double_init(admin: signer) {
        ai_YieldFlow::native_usdc_vault_fa::initialize(&admin);
    ...
    public fun test_admin_yield_only(admin: signer, user: signer) {
        ...
        // ai_YieldFlow::native_usdc_vault_fa::deposit(&user, 1000, admin_addr);
        
        // Only admin can add yield; ensure yield is added properly
        // ai_YieldFlow::native_usdc_vault_fa::add_yield(&admin, user_addr, 12_345);
        // let (_, yield_earned, _, _) = ai_YieldFlow::native_usdc_vault_fa::get_user_position(user_addr);
    ...
    #[expected_failure(abort_code = 2, location = ai_YieldFlow::native_usdc_vault_fa)]
    public fun test_unauthorized_yield_add(admin: signer, non_admin: signer) {
        ai_YieldFlow::native_usdc_vault_fa::initialize(&admin);
        let user_addr = @0x4;
        // Non-admin should not be able to add yield
        ai_YieldFlow::native_usdc_vault_fa::add_yield(&non_admin, user_addr, 1);
    }
    ...
    public fun test_stats_views_empty(admin: signer) {
        ...
        let (total_deposits, total_yield) = ai_YieldFlow::native_usdc_vault_fa::get_vault_stats(admin_addr);
        assert!(total_deposits == 0, 40);
        assert!(total_yield == 0, 41);
        let no_user = @0xcafe99;
        let res = ai_YieldFlow::native_usdc_vault_fa::get_total_balance(no_user);
        assert!(res == 0, 42);
    }
}
